...
**Skills** *(optional)* — select one or more skills to attach to the project (e.g. `duplocloud`). Skills extend what the AI agent can do when working on tickets inside this project.
...
> **Authentication:** When prompted, provide your DuploCloud platform URL and user access token:
> `authenticate to duplo using url: https://<helpdesk-url> and token: <user-access-token>`
