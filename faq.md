...
For enhanced security, users can set up Multi-Factor Authentication (MFA) for OpenVPN access. This process involves logging into Duplo, navigating to the user profile, and following the VPN URL to enter credentials. Upon the first login, a barcode will be displayed for device registration. After downloading and adding the profile to OpenVPN Connect, MFA will be required for subsequent logins, ensuring an additional layer of security for VPN access.
