...
Use a valid API token (`Bearer {token}`) in the Authorization header. You can generate a permanent API token from the DuploCloud Portal: see the [API Token](../../automation-platform/access-control/api-and-session-management/api-tokens.md#permanent-api-tokens) page for instructions.
