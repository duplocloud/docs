...
DuploCloud allows you to automatically shut down Tenant resources, such as EC2 instances and RDS databases, based on defined schedules. Auto Scaling Groups (ASGs) and RDS clusters (Aurora or multi-instance clusters) are not supported.

This feature is available only for AWS. It is useful for managing costs and conserving resources in development, test, or other non-production environments.

To schedule shutdowns for instances in a Tenant, complete these three steps:

1. **Define Periods**: Define specific time ranges and days of the week when shutdowns should occur.
2. **Create Schedules**: Group one or more periods and apply them to a particular resource type (EC2 or RDS).&#x20;
3. **Assign Schedules**: Assign the schedules to all instances of a resource type in the Tenant  or to specific individual instances. Schedules can be either:
   * **Group schedules:** Apply to all instances of the selected resource type in a Tenant
   * **Individual schedules**: Apply only to the selected instance(s) (EC2 or RDS)

{% hint style="info" %}
**Notes:**

* Each Tenant can have one Group and one Individual schedule per resource type (EC2 or RDS).
* You can create and use multiple schedules per Tenant.
{% endhint %}
...
{% hint style="info" %}
It may take up to 10 minutes after saving before the **Scheduler** tab becomes visible under **Administrator → Tenants**.
{% endhint %}
...
Schedules define how and when shutdown periods are applied to a specific resource type within a Tenant.&#x20;

You can create two types of schedules:

* **Group schedules:** Applies to all EC2 or RDS instances in the Tenant
* **Individual schedules:** Applies only to the specific EC2 or RDS instances selected

**Multiple schedules per Tenant are supported**, but only one active Group and one Individual schedule per resource type can be assigned at a time.
...
7. Repeat steps 1-6 to add additional schedules to a Tenant.
...
{% hint style="info" %}
The scheduler runs on AWS’s standard schedule interval, which checks for updates every 5 minutes, therefore it may take up to 5 minutes for the schedule to take effect.
{% endhint %}
