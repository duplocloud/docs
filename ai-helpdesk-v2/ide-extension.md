...
The plugin works identically whether you're using Claude Code in a terminal (`claude`) or through the Claude Code extension in your IDE. Both surfaces share the same commands, state, and workflow.
...
Clone the plugin into the Claude Code plugins directory:

```bash
git clone <repo-url> ~/.claude/plugins/duplo
```

The plugin activates automatically on your next Claude Code session. The first time you submit a prompt, it installs its CLI tools to `~/.duplocloud/bin/` — no manual setup required.
