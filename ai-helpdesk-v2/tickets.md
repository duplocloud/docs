...
Tickets can be **standalone** — created on demand for ad hoc tasks or troubleshooting — or **part of a Project**, where they are generated automatically from the stages of an approved implementation plan. In both cases, the interaction model is the same: a dedicated channel between you and an assigned Agent, with a full audit trail of everything that happens.

## Creating a Ticket

To begin a new interaction, create a Ticket and select an Agent with the appropriate domain expertise — such as Kubernetes operations, observability, or deployment troubleshooting. A ticket can include input from multiple Agents, allowing for collaboration across different domains within the same session.

To create a Ticket, complete the following steps:

1. Navigate to **AI Helpdesk**.
...
3. In the **Message to Agent** field, describe your task or problem, or choose one of the provided prompts.
