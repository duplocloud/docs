...
DuploCloud includes several pre-built, production-ready Agents that handle common DevOps and infrastructure management tasks. These Agents integrate seamlessly with your existing infrastructure and can be deployed immediately to automate routine operations and troubleshooting workflows.

Agents operate within the boundaries defined by Providers and Scopes. Providers store the credentials for your cloud accounts, Kubernetes clusters, observability tools, and other services. Scopes define exactly which resources within those Providers an Agent can access. When an Agent is assigned to an Engineer, it inherits only the Scopes granted to that Engineer — ensuring precise, auditable access control.

Agents are used both for ad hoc tasks and as part of Projects. Within a Project, different tickets can be assigned to different specialized Agents — for example, an IaC ticket might be handled by the IaC Agent while an observability ticket is routed to the Observability Agent. You can also access Agents through the AI Helpdesk by creating a ticket and assigning it to the appropriate Agent.
...
While these out-of-the-box Agents cover many common use cases, DuploCloud also supports custom Agents tailored to your specific workflows and tools. For assistance with Agent customization or integration with additional tools, contact the DuploCloud team.
